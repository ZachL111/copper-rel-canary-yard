...
`copper-rel-canary-yard` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for canary scenarios with framed sample traffic, bounds and ordering tests, and bounded memory input sets.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 3.
